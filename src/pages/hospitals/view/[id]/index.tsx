...
import { deletePatientById } from 'apiSdk/patients';
...
function HospitalViewPage() {
  ...
  const patientHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('patient', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/patients/create?hospital_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.patient?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/patients/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>
